...
> PLAN-006 | v2.0 | 2026-04-07 | status: draft
...
| `S2` | 修正字段数量检查 `len >= 2` | S1 | `monitor/collector.py` | ⏳ 待执行 |
| `S3` | 处理GPU使用率获取（单独查询或设为0） | S2 | `monitor/collector.py` | ⏳ 待执行 |
| `S4` | 本地测试验证修复效果 | S3 | 测试输出截图 | ⏳ 待执行 |
| `S5` | 部署到GPU服务器并验证 | S4 | 机器人 `/info` 实际输出 | ⏳ 待执行 |
